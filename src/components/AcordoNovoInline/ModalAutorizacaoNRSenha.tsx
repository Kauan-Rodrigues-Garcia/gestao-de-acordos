/**
 * ModalAutorizacaoNRSenha — a janela com login do líder, hoje só na EDIÇÃO.
 *
 * ## Por que sobreviveu
 *
 * A tela de NOVO acordo passou ao fluxo por solicitação (`ModalAutorizacaoNR`):
 * o operador pede, o líder decide pela gaveta, e o SERVIDOR grava o acordo na
 * aprovação — o operador nem precisa estar na tela.
 *
 * A tela de EDIÇÃO não pôde acompanhar. Ali o que se grava não é um acordo
 * novo: é um `update` no acordo que já existe, com o recálculo de parcelamento
 * que `gravar()` faz (regra dos 40 %, entrada, valor_total). Reproduzir isso no
 * servidor criaria um SEGUNDO caminho de gravação de acordo, para divergir do
 * primeiro no primeiro ajuste — exatamente o defeito que este projeto já pagou
 * caro em `escopoAnalitico` e em `projecaoMetas`.
 *
 * Enquanto esse recálculo não sair de dentro do componente, editar um acordo
 * para um NR/Código já vinculado continua exigindo o líder na máquina. É um
 * caminho bem mais raro que registrar — e é o único que ainda pede senha.
 */
...
import type { ModalAutorizacaoNRSenhaProps } from './types';

export function ModalAutorizacaoNRSenha({
