/**
 * PetNomeVotacao — janela central que abre uma vez por sessão para todo usuário
 * que ainda não votou no nome do mascote (Aura / Lupi / Albi).
 *
 * O voto fica salvo por usuário no banco (com a empresa, p/ apurar BookPlay e
 * PaguePlay separadamente). Depois de confirmar, não aparece mais. O placar é
 * privado — o usuário só vê um agradecimento, nunca a parcial.
 *
 * Degradação graciosa: sem a migration aplicada (getMeuVotoNome → indisponivel),
 * a janela simplesmente não abre.
 */
...
type Fase = 'carregando' | 'oculto' | 'votando' | 'obrigado';

export function PetNomeVotacao() {
  ...
  // Decide uma única vez por sessão: mostra só se ainda não votou.
  useEffect(() => {
    if (!perfil?.id || decididoRef.current) return;
    let vivo = true;
              ...
              <div className="px-5 pb-5 pt-1 space-y-4">
                <div className="text-center space-y-1.5">
                  <h2 className="text-base font-bold text-foreground">
                    🗳️ Ajude a escolher o nome do mascote!
                  </h2>
                  <p className="text-[12.5px] text-muted-foreground leading-relaxed">
                    Em breve ele ganha vida: conforme seus recebimentos crescerem,
                    você vai poder cuidar, vestir e ver seu mascote evoluir.
                    Antes disso, qual nome combina mais com ele?
                  </p>
                </div>
