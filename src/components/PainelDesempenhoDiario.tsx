...
export function PainelDesempenhoDiario({
  temLogicaDiretoExtra = false,
  operadorFiltroExterno,
}: PainelDesempenhoDiarioProps) {
  ...
  const ticketMedio = data.qtdPagos > 0
    ? data.valorRecebido / data.qtdPagos
    : 0;
  ...
  return (
    <>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        className={cn(
          'fixed bottom-6 left-6 z-40 flex items-center gap-2 px-4 py-2.5 rounded-2xl shadow-xl border font-semibold text-sm transition-colors',
          aberto
            ? 'bg-violet-600 border-violet-500 text-white'
            : 'bg-card border-violet-500/40 text-violet-500 hover:bg-violet-500/10 hover:border-violet-500',
        )}
        title="Painel de Desempenho Diário"
      >
        <BarChart2 className="w-4 h-4 shrink-0" />
        <span className="hidden sm:inline">Desempenho do Dia</span>
      </motion.button>
        ...
        {aberto && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-20 left-6 z-40 w-[390px] max-w-[calc(100vw-3rem)] max-h-[80vh] flex flex-col rounded-2xl border border-border/80 bg-card shadow-2xl overflow-hidden"
            >
              {/* Header */}
              ...
              <div className="px-5 py-3 border-b border-border/60 shrink-0 space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {!isToday && (
                    <Button
                      variant="outline" size="sm" className="h-7 text-xs px-2.5 rounded-lg gap-1 text-violet-500 border-violet-500/30 hover:bg-violet-500/10 shrink-0"
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-2.5">
                        ...
                        <StatCard
                          ...
                          sub={`Sobre bruto de ${formatCurrency(data.valorRecebido)}`}
                        />
                      </div>

                      {/* Direto + Extra (apenas se ativo) */}
                      {temLogicaDiretoExtra && (
                        <div className="grid grid-cols-2 gap-2.5">
                          <StatCard
                            label="H.O. Direto"
