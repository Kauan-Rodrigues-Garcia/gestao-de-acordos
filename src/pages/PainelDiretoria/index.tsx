...
import {
  ...
  Building2, RefreshCw, CreditCard, Landmark, QrCode,
...
import { TIPO_ICONS, TIPO_CORES, TIPO_LABELS_DISPLAY, EVOL_AGENDADO, EVOL_RECEBIDO } from './types';
...
export default function PainelDiretoria() {
  ...
  } = useSetoresExtras(empresa?.id, isPP);
  ...
  const ticketMedio = totalPagosMes > 0 ? Math.round(valorRecebidoMes / totalPagosMes) : 0;
  const inadimplencia = valorAgendadoMes > 0 ? Math.round((valorNaoPago / valorAgendadoMes) * 100) : 0;
  const deltaAgendado = mesAnterior && mesAnterior.valorAgendado > 0 ? Math.round(((valorAgendadoMes - mesAnterior.valorAgendado) / mesAnterior.valorAgendado) * 100) : null;
  const deltaRecebido = mesAnterior && mesAnterior.valorRecebido > 0 ? Math.round(((valorRecebidoMes - mesAnterior.valorRecebido) / mesAnterior.valorRecebido) * 100) : null;
  ...
  const distribuicaoPorTipo = useMemo(() => {
    ...
    return Object.entries(map)
      .map(([tipo, d]) => ({ tipo, name: TIPO_LABELS_DISPLAY[tipo] ?? tipo, value: d.agendado, recebido: d.recebido, qtd: d.qtd, fill: TIPO_CORES[tipo] ?? '#94a3b8' }))
  ...
  const projecaoMes = diasPassados > 0 && valorBaseProjecao > 0 ? Math.round((valorBaseProjecao / diasPassados) * diasNoMes) : 0;
  const projecaoBruta = diasPassados > 0 && valorRecebidoMes > 0 ? Math.round((valorRecebidoMes / diasPassados) * diasNoMes) : 0;
  ...
  const agora = hoje.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
  ...
  return (
    <div className="space-y-6 p-4 md:p-6 max-w-[1600px] mx-auto">
      ...
      >
        ...
        <div className="px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Button variant="outline" size="sm" onClick={() => { refetch(); reloadSetoresExtras(); }}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
        {loading ? <KpiSkeletons count={5} /> : (
          <>
            <KpiCard label={isPP ? 'H.O. recebido no mês' : 'Recebido no mês'} value={isPP ? formatBRL(valorHOMes) : formatBRL(valorRecebidoMes)} sub={isPP ? `Bruto: ${formatBRL(valorRecebidoMes)} · ${totalPagosMes} pagos` : `${totalPagosMes} acordos pagos`} icon={DollarSign} color="text-success" bg="bg-success/10" delta={deltaRecebido !== null ? { value: `${deltaRecebido > 0 ? '+' : ''}${deltaRecebido}%`, up: deltaRecebido >= 0 } : undefined} delay={0} />
            ...
            <KpiCard label="Taxa de conversão" value={`${txConversao}%`} sub={`${totalPendentes} pendentes`} icon={CheckCircle2} color="text-chart-3" bg="bg-chart-3/10" delay={0.24} />
          </>
        )}
      </div>
      ...
      {isPP && !loading && valorRecebidoMes > 0 && (
        <ReceitaDistribuicaoPP
          valorRecebidoMes={valorRecebidoMes} valorHOMes={valorHOMes} valorHOAgendado={valorHOAgendado}
          valorNaoPago={valorNaoPago} valorCorenMes={valorCorenMes} valorCofenMes={valorCofenMes}
          valorCorenAge={valorCorenAge} valorCofenAge={valorCofenAge} meta={meta} percMeta={percMeta}
        />
      )}
      ...
      {!loading && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <KpiCard label="Ticket médio (pagos)" value={formatBRL(ticketMedio)} sub="por acordo pago" icon={Activity} color="text-chart-1" bg="bg-chart-1/10" delay={0.22} />
            ...
            <KpiCard label={isPP ? 'Projeção H.O./mês' : 'Projeção do mês'} value={projecaoMes > 0 ? formatBRL(projecaoMes) : '—'} sub={isPP && projecaoBruta > 0 ? `Bruto: ${formatBRL(projecaoBruta)} · ${diasPassados}/${diasNoMes} dias` : `Base: ${diasPassados} de ${diasNoMes} dias`} icon={Target} color="text-chart-5" bg="bg-chart-5/10" delay={0.30} />
            <KpiCard label="Pendentes" value={String(totalPendentes)} sub={`≈ ${formatBRL(valorAgendadoMes - valorRecebidoMes - valorNaoPago)} a verificar`} icon={Clock} color="text-chart-4" bg="bg-chart-4/10" delay={0.34} />
          </div>
        </>
      )}
      ...
      {meta && !loading && (
        <MetaSection
          meta={meta} percMeta={percMeta} isPP={isPP}
          valorHOMes={valorHOMes} valorRecebidoMes={valorRecebidoMes}
            ...
            <div className="relative rounded-2xl border border-border/40 bg-card/80 backdrop-blur-sm overflow-hidden">
              ...
              <div className="p-4 relative">
                <ResponsiveContainer width="100%" height={240}>
                  <RechartsPie>
                    <Pie data={distribuicaoPorTipo} cx="50%" cy="50%" innerRadius={65} outerRadius={95} paddingAngle={4} dataKey="value" nameKey="name" strokeWidth={0}>
                      {distribuicaoPorTipo.map((entry, i) => <Cell key={i} fill={entry.fill} />)}
                    </Pie>
                ...
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ top: '-10px' }}>
                  <div className="text-center">
                    ...
                    <p className="text-sm font-extrabold font-mono text-foreground leading-none mt-0.5">{formatBRL(distribuicaoPorTipo.reduce((s, d) => s + d.value, 0))}</p>
                    <p className="text-[10px] text-muted-foreground mt-0.5">{distribuicaoPorTipo.reduce((s, d) => s + d.qtd, 0)} acordos</p>
                  </div>
                </div>
              </div>
            </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2">
                            <div className="flex-1 h-1.5 rounded-full bg-muted overflow-hidden">
                              <motion.div className="h-full rounded-full" style={{ background: d.fill }} initial={{ width: 0 }} animate={{ width: `${percRec}%` }} transition={{ duration: 0.7, ease: 'easeOut', delay: i * 0.05 }} />
                            </div>
                            <span className={cn('text-[10px] font-bold w-8 text-right tabular-nums', percRec >= 80 ? 'text-success' : percRec >= 50 ? 'text-warning' : 'text-muted-foreground')}>{percRec}%</span>
                          </div>
                          <p className="text-[10px] text-muted-foreground mt-0.5">{d.qtd} acordos · <span className="text-success font-semibold">{formatBRL(d.recebido)}</span> recebido</p>
                        </div>
                      </div>
                    </div>
                  );
      ...
      <RankingsSection porEquipe={porEquipe} porOperador={porOperador} loading={loading} />
          ...
          >
            ...
            <div className="p-5">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: 'Agendado', atual: isPP ? valorHOAgendado : valorAgendadoMes, anterior: isPP ? mesAnterior.valorAgendado * PP_HO_PERCENTUAL : mesAnterior.valorAgendado, delta: deltaAgendado, totalAtual: totalAcordosMes, totalAnterior: mesAnterior.totalAcordos, labelAtual: isPP ? 'H.O. agendado atual' : 'Agendado atual', labelAnterior: isPP ? 'H.O. agendado anterior' : 'Agendado anterior', color: 'primary' as const },
                  { label: 'Recebido', atual: isPP ? valorHOMes : valorRecebidoMes, anterior: isPP ? mesAnterior.valorRecebido * PP_HO_PERCENTUAL : mesAnterior.valorRecebido, delta: deltaRecebido, totalAtual: totalPagosMes, totalAnterior: null, labelAtual: isPP ? 'H.O. recebido atual' : 'Recebido atual', labelAnterior: isPP ? 'H.O. recebido anterior' : 'Recebido anterior', color: 'success' as const },
                  ...
                  <div key={label} className="space-y-2">
                    ...
                    <div className={cn('p-4 rounded-xl border', color === 'primary' ? 'border-primary/20 bg-primary/5' : 'border-success/20 bg-success/5')}>
                      ...
                      <p className="text-[10px] text-muted-foreground mt-1">{totalAtual} {label === 'Agendado' ? 'acordos' : 'pagos'}{isPP && label === 'Agendado' ? ` · bruto ${formatBRL(valorAgendadoMes)}` : ''}{isPP && label === 'Recebido' ? ` · bruto ${formatBRL(valorRecebidoMes)}` : ''}</p>
                    </div>
                    {delta !== null && (
