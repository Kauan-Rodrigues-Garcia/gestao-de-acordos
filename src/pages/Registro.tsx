 ...
 *   - Senha
 *
 * O usuário entra como líder do tenant para visualizar os acordos existentes.
 *
...
export default function Registro() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const { data: usuarioExistente, error: lookupError } = await supabase.rpc('buscar_email_por_usuario_empresa', {
        p_usuario: usuario.trim().toLowerCase(),
        p_empresa_slug: tenantSlug,
      });

      if (!lookupError && usuarioExistente) {
        setErro('Este usuário já está cadastrado nesta empresa. Tente fazer login ou use outro usuário.');
        return;
      }
