...
export function participaDaCampanha(pessoa: PessoaDesafio, desafio: Desafio): boolean {
  const { setores, equipes, operadores } = desafio.regra.participantes;
 ...
 *
 * A distância é em DINHEIRO mesmo quando o critério é outro: "faltam R$ 480
 * para alcançar o 3º" é a frase que o operador entende, e ela é verdadeira nos
 * três critérios enquanto a meta for a mesma para todos — que é como a
 * configuração de campanha trabalha.
 */
function posicionar<T extends { recebido: number; posicao: number;
                                paraUltrapassar: number | null }>(lista: T[]): void {
  lista.forEach((item, i) => {
    item.posicao = i + 1;
    const acima = i > 0 ? lista[i - 1] : null;
    item.paraUltrapassar = acima ? Math.max(acima.recebido - item.recebido, 0) : null;
  });
}
...
export function calcularDesafio(params: ParametrosCalculo): ResultadoDesafio {
  const { desafio, dados, ocultos, filtroSetorId } = params;
  const { regra } = desafio;
  ...
  const elegiveis = dados.participantes.filter(p => {
    ...
    if (filtroSetorId && !p.setores.includes(filtroSetorId)) return false;
    return participaDaCampanha(p, desafio);
  });

  const metaIndividual = regra.metaIndividual;
  ...
  const individual: ResultadoParticipante[] = elegiveis.map(pessoa => {
    ...
    const falta = faltaParaMeta(recebido, metaIndividual);
    return {
      pessoa,
      posicao: 0,
      recebido,
      ...
      meta: metaIndividual,
      falta,
      progresso: progressoDaMeta(recebido, metaIndividual),
      bateuMeta: !!metaIndividual && metaIndividual > 0 && recebido >= metaIndividual,
  ...
  const metaEquipe = regra.metaEquipe;
  ...
  const equipes: ResultadoEquipe[] = [...porEquipe.entries()].map(([id, integrantes]) => {
    ...
    const falta    = faltaParaMeta(recebido, metaEquipe);
    return {
      equipeId: id,
      ...
      meta: metaEquipe,
      falta,
      progresso: progressoDaMeta(recebido, metaEquipe),
      bateuMeta: !!metaEquipe && metaEquipe > 0 && recebido >= metaEquipe,
      paraUltrapassar: null as number | null,
