...
export interface ResultadoImportacao {
  ...
  /**
   * Este setor/mês já é do relatório 59?
   *
   * Não muda nada do que foi gravado — o 58 alimenta igual. Serve para a tela
   * dizer que aqueles números ainda vão passar pela conferência do 59 na
   * próxima entrada dele, que é quando a diferença de data entre as duas fontes
   * se resolve. Ver `fn_analitico_fonte_do_setor`.
   */
  sincronizadoPelo59?: boolean;
  erros: string[];
}
...
export async function importarLoteAnalitico(
  ...
  /*
   * ## O 58 continua alimentando; o 59 reconcilia depois
   *
   * Houve uma versão em que o 58 virava prévia INERTE nos setores do 59 — lia o
   * arquivo e não gravava nada. Estava errado, e o pedido é explícito: «o 58 é
   * pra ir atualizando os valores também, igual é hoje, porém ao entrar o 59
   * sincroniza tudo».
   *
   * Então o 58 segue sendo a alimentação rápida do dia, importada por setor, e
   * o 59 é a autoridade que corrige tudo quando chega — pelo gatilho
   * `trg_mestre_aplicar_no_analitico` (migration 20260914…), que reaplica a
   * projeção sobre cada setor do 59 no instante em que o lote é promovido.
   *
   ...
   * chave de `idx_analitico_unicidade` é outra e o 58 insere ao lado em vez de
   * colidir.
   *
   ...
   *
   * E some sozinho na próxima entrada do 59, que apaga e regrava. Por isso a
   * sincronização é gatilho e não botão: se dependesse de alguém lembrar de
   * clicar, esses R$ 40 mil virariam permanentes.
   *
   * O que se consulta aqui não muda o que é gravado — serve só para a tela
   * dizer, depois, que aqueles números ainda passam pelo 59.
   */
  const rows = todasAsLinhas;
  const mesesDoArquivo = [...new Set(rows.map(r => r.mes_referencia))];
  let sincronizadoPelo59 = false;
  if (setorImportacaoId) {
    for (const mes of mesesDoArquivo) {
      ...
      /* Falha aqui é cosmética: o aviso não aparece e a importação segue igual.
         Não pode virar erro — a gravação não depende desta resposta. */
      if (error) console.warn('[analitico] fonte do setor não verificada:', error.message);
      else if (data === 'relatorio_59') sincronizadoPelo59 = true;
    }
  }
  ...
  const registrarResumo = (reconciliou: boolean) => {
    ...
    void registrarLog({
      ...
      detalhes: {
        ...
        sincronizado_pelo_59: sincronizadoPelo59,
  ...
  if (!leituraCompleta) {
    ...
    return { inseridos, duplicados, atualizados, removidos, tiposPreenchidos, sincronizadoPelo59, erros };
  }
