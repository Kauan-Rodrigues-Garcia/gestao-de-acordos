 ...
 * - Nome completo
 * - Usuário (username para login)
 * - E-mail (opcional — se omitido, gera email fictício @interno.sistema)
 * - Senha
 *
 ...
 *
 * CORREÇÃO:
 * - Chama beginSignUp() antes do signUp para prevenir login automático
 *   disparado pelo onAuthStateChange quando confirmação está desabilitada.
 * - Faz signOut explícito após cadastro bem-sucedido como segurança adicional.
 * - Redireciona para tela de login após sucesso (não loga automaticamente).
 */
...
export default function Registro() {
  ...
  const { beginSignUp, signOut } = useAuth();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // ── CORREÇÃO: sinalizar início do signUp ANTES de chamar supabase.auth.signUp
      // Isso faz com que o onAuthStateChange ignore o SIGNED_IN automático e
      // não logue o usuário sem que ele passe pela tela de login.
      beginSignUp();
      ...
      // ── CORREÇÃO: garantir que nenhuma sessão fique ativa após o cadastro.
      // O onAuthStateChange já trata isso via isSigningUp, mas fazemos signOut
      // explícito como camada de segurança adicional.
      await signOut();
          ...
          <Card className="border-success/30 shadow-lg">
            <CardContent className="p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-6">
                Sua conta foi criada com sucesso e já está vinculada a{' '}
                <strong className="text-foreground">{empresa?.nome ?? branding.shortName}</strong>.
              </p>
              <p className="text-xs text-muted-foreground mb-6">
                Use seu <strong>usuário</strong> e <strong>senha</strong> para entrar.
              </p>
