-- ═══════════════════════════════════════════════════════════════════════════
-- APLICAR NO SQL EDITOR — Pix Automático, 09/09/2026
-- ═══════════════════════════════════════════════════════════════════════════
--
-- As duas migrations do commit 68edaf0, na ordem, num arquivo só para colar.
-- Cada uma abre e fecha a PRÓPRIA transação (BEGIN ... COMMIT): se a segunda
-- falhar, a primeira continua aplicada, e é assim que se quer — a primeira
-- (expurgo agendado) não depende da segunda.
--
--   1. 20260909100000_pix_expurgo_desaprovados_agendado.sql
--   2. 20260909110000_pix_nr_duplicidade_por_pessoa.sql
--
-- A segunda ABORTA DE PROPÓSITO, listando os NRs, se encontrar duplicata sem
-- autorização já no banco. Nesse caso nada é aplicado (a transação inteira
-- volta atrás) e a mensagem diz quais NRs resolver.
--
-- Depois de aplicar, o front do commit 68edaf0 tem de estar publicado:
-- `fn_pix_nr_pedido_decidir` passou de 3 para 4 argumentos e a versão de 3
-- é removida aqui.
--
-- Este arquivo é uma CÓPIA para colar. A fonte são as migrations em
-- supabase/migrations/ — se precisar mudar algo, mude lá.
-- ═══════════════════════════════════════════════════════════════════════════


-- ─────────── 1 de 2: 20260909100000_pix_expurgo_desaprovados_agendado.sql ───────────
...
-- ─────────── 2 de 2: 20260909110000_pix_nr_duplicidade_por_pessoa.sql ───────────
