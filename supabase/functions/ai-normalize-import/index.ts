...
Deno.serve(async (req) => {
  ...
  const userPrompt = [
    ...
    `Referência de "hoje": ${todayISO}.`,
    ``,
    `DETECÇÃO DE FORMATO:`,
    `- Se uma linha contiver APENAS uma data (ex: "01/02", "03/02/2025", "5/2"), ela é um MARCADOR DE BLOCO.`,
    `  Não gere record para ela. Use essa data como "vencimento" para todos os registros abaixo, até o próximo marcador.`,
    `- Linhas de cabeçalho repetidas (contendo palavras como NR, VALOR, WHATS, STATUS, NOME, VENC) devem ser IGNORADAS.`,
    ...
    `- vencimento: formato YYYY-MM-DD. Datas no formato brasileiro DD/MM: use o ano de "${todayISO}" como referência; se a data DD/MM já passou e o contexto indica vencimento futuro, use o próximo ano.`,
    `- status deve preferir: pendente, pago, verificar, vencido, cancelado, em_acompanhamento.`,
    `- tipo deve preferir: boleto, pix, cartao. Textos como "PIX AUTO" ou "RECORRENTE" podem indicar tipo ou ir para observacoes.`,
    `- valor deve ser número decimal (ex: 1234.56). Converter "R$ 266,66" → 266.66.`,
    `- whatsapp: manter formato original com DDD, ex: (93)99158-1981.`,
    `- A última coluna da planilha pode conter tipo, observações ou informações adicionais — extraia para tipo ou observacoes conforme adequado.`,
    ``,
    `TSV:`,
    tsv,
  ].join('\n');
