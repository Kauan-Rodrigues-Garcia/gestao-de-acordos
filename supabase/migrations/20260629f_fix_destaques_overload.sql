-- Remove a versão antiga (2 parâmetros) que conflita com a nova (3 parâmetros).
-- O CREATE OR REPLACE não substitui quando a assinatura muda — cria uma sobrecarga,
-- e o Postgres fica ambíguo ao chamar sem o terceiro parâmetro.
DROP FUNCTION IF EXISTS public.fn_analitico_destaques_dia(UUID, TEXT);

-- Recria com o terceiro parâmetro opcional (DEFAULT NULL).
