-- ═══════════════════════════════════════════════════════════════════════════
-- A variável morta sai do detalhe por operador
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Irmã da 20260913233513. Aquela versão declarava `v_mes uuid` e nunca usava:
-- a função passou a calcular a data inline e a declaração ficou para trás, com
-- o tipo errado por cima.
--
-- Não fazia nada em tempo de execução. Mas uma variável com nome de data e
-- tipo de uuid é exatamente o que faz alguém perder meia hora daqui a seis
-- meses tentando entender para que ela serve.
--
-- Escrita: nenhuma.
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  if (select pg_get_functiondef(p.oid) from pg_proc p
       ...
       where n.nspname = 'public' and p.proname = 'fn_mestre_operador_detalhe')
     like '%v_mes uuid%' then
    raise exception 'A variavel morta continua la.';
  end if;
end
$prova$;
