-- ═══════════════════════════════════════════════════════════════════════════
-- O detalhe da diferença acompanha a regra de colchão do 58
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Par da 20260913172128. Aquela migration mudou o que o comparável soma; esta
-- faz a lista de NR somar a MESMA coisa. Sem isso o «não explicado» passaria a
-- acusar um buraco do tamanho exato do colchão do mês — R$ 77.450,26 no
-- Receptivo em setembro/2026.
--
-- Três mudanças, todas consequência:
--
--   • a CTE m troca fn_mestre_conta_na_meta por fn_mestre_esta_no_58. É ela que
--     faz o lado do 59 somar exatamente mestre_comparavel, e é isso que permite
--     o «não explicado» chegar a zero;
--
--   • linhas_emp NÃO muda. Ela reproduz o empréstimo de
--     fn_mestre_resumo_grupos, que é regra de META — trocar o predicado ali
--     faria a parcela da tela discordar do valor que ela detalha;
--
--   • a parcela «Colchão fora da meta» vira «Colchão, que o 58 não guarda», que
--     é o que ela de fato mede desde a migration anterior.
--
-- Escrita: nenhuma. Um create or replace de função de leitura.
-- ═══════════════════════════════════════════════════════════════════════════
  ...
  m as (
    select r.nr_documento as nr,
     ...
     where r.empresa_id = p_empresa_id
       ...
       and fn_mestre_esta_no_58(r.colchao, r.dt_pgto)
  ...
  select jsonb_build_object(
    ...
    'estrutura', jsonb_build_array(
      ...
      jsonb_build_object('chave','colchao_fora','valor', v_cmp.mestre_colchao_fora,
        'rotulo','Colchao, que o 58 nao guarda',
        'nota','O parser do 58 manda colchao para analitico_colchao_fora_meta, menos na janela 01-14/08/2026. No 59 ele conta na meta de setembro em diante — por isso entra em mestre_total e fica fora do comparavel.'),
...
comment on function public.fn_mestre_diferenca_detalhe(uuid, text, uuid, integer) is
  ...
  '(carteira, fn_mestre_esta_no_58, sem somente_geral, COM Extra). Separa o '
  'ESTRUTURAL (fora_da_comparacao) do DIVERGENTE (nrs). So leitura.';

-- ── Prova ──────────────────────────────────────────────────────────────────
--
-- O predicado do 58 nas datas que decidem, e a divergência proposital com a
-- regra de meta. Se alguma falhar, a transação volta inteira.

do \$
begin
  if not fn_mestre_esta_no_58(false, date '2026-09-20') then
    raise exception 'Linha sem colchao tem de estar no 58.';
  end if;
  if not fn_mestre_esta_no_58(true, date '2026-08-10') then
    raise exception 'Colchao de 10/08/2026 entrou no analitico e tem de contar.';
  end if;
  if fn_mestre_esta_no_58(true, date '2026-08-20') then
    raise exception 'Colchao de 20/08/2026 foi para a tabela de fora.';
  end if;
  if fn_mestre_esta_no_58(true, date '2026-09-05') then
    raise exception 'Colchao de setembro NAO entra em analitico_recebimentos.';
  end if;
  if fn_mestre_esta_no_58(true, date '2026-07-15') then
    raise exception 'Colchao de julho nunca entrou.';
  end if;
  -- Em agosto as duas regras coincidem; de setembro em diante, nao.
  if fn_mestre_esta_no_58(true, date '2026-08-10') <> fn_mestre_conta_na_meta(true, date '2026-08-10') then
    raise exception 'As duas regras deveriam coincidir em agosto.';
  end if;
  if fn_mestre_esta_no_58(true, date '2026-09-10') = fn_mestre_conta_na_meta(true, date '2026-09-10') then
    raise exception 'As duas regras deveriam divergir em setembro.';
  end if;
end
\$;
