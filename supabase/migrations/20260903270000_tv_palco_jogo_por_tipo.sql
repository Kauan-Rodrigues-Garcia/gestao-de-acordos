-- ============================================================================
-- Modo TV 2.0 — fase 3: o palco enxerga o jogo certo
-- ============================================================================
--
-- Duas mudanças no ramo de sorteio de `fn_tv_palco`; o resto fica letra por
-- letra.
--
-- 1. A fonte passa a buscar o jogo DO SEU TIPO. Ela já diz em `config.modelo`
--    se é roleta, bingo ou sorteio de pessoa, e antes a consulta ignorava isso
--    e pegava o mais recente do setor, qualquer que fosse — abrir um bingo
--    fazia a roleta da cena ao lado passar a mostrar números.
--
-- 2. `config` do jogo viaja junto. É nele que moram o layout da roda, o "sai
--    quem já saiu" e até onde vai o bingo. O desenho é do palco; a decisão, de
--    quem configurou.
-- ============================================================================
      ...
      SELECT
        ...
        jsonb_build_object(
          ...
          'dados', CASE fo.tipo
            ...
            /*
             * O jogo mais recente do setor, DO TIPO que a fonte pede.
             *
             * A fonte ja diz em `config.modelo` se e roleta, bingo ou sorteio
             * de pessoa. Antes a consulta ignorava isso e pegava o mais recente
             * do setor, qualquer que fosse — abrir um bingo fazia a roleta da
             * cena ao lado passar a mostrar numeros.
             *
             * `config` viaja junto: e nele que estao o layout da roda, o "sai
             * quem ja saiu" e ate onde vai o bingo. O desenho e do palco.
             */
            WHEN 'sorteio' THEN (
              SELECT jsonb_build_object(
                ...
                'girado_por_nome', s.girado_por_nome
              )
                FROM public.tv_sorteios s
               WHERE s.setor_id = v_tela.setor_id
                 AND s.tipo = COALESCE(NULLIF(fo.config ->> 'modelo', ''), 'roleta')
