-- Ouvidoria — complemento da 20260717b.
--
-- 1. Garante o valor 'ouvidoria' no enum (idempotente; é a causa do 400
--    "invalid input value for enum perfil_usuario" ao salvar o cargo quando
--    a 20260717b ainda não foi aplicada).
-- 2. fn_criar_perfil_novo_usuario: a allowlist de perfis do trigger de signup
--    não incluía 'ouvidoria' — usuário NOVO criado já com esse cargo cairia
--    silenciosamente em 'operador'. Função recriada com o cargo na lista
--    (única mudança em relação à versão de fix_novos_cargos_trigger_rls).
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TYPE public.perfil_usuario ADD VALUE 'ouvidoria';
  END IF;
END;
$$;

-- ─── 2. Trigger de signup aceita 'ouvidoria' ────────────────────────────────
