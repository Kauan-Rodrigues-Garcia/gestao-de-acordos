...
-- ================================================
-- TABELAS DE FUNDAÇÃO LEGADAS
-- ================================================
-- Estas tabelas já existiam em produção, mas nunca tiveram CREATE TABLE no
-- repositório. Mantê-las na primeira migration permite que os scripts
-- seguintes (a começar por 02_seed_setores.sql) sejam reaplicados do zero.
...
CREATE INDEX IF NOT EXISTS idx_lixeira_empresa
  ON public.lixeira_acordos(empresa_id);
CREATE INDEX IF NOT EXISTS idx_lixeira_operador
  ON public.lixeira_acordos(operador_id);
CREATE INDEX IF NOT EXISTS idx_lixeira_nr
  ON public.lixeira_acordos(nr_cliente);
ALTER TABLE public.lixeira_acordos ENABLE ROW LEVEL SECURITY;

-- Tabela anterior a `perfis`, ainda presente no schema de produção. O app não
-- depende dela, mas o baseline deve reproduzir o banco sem objetos órfãos.
CREATE TABLE IF NOT EXISTS public.profiles (
  id                 UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  nome               TEXT,
  setor              TEXT,
  perfil             TEXT DEFAULT 'setor'
    CHECK (perfil IN ('admin', 'setor')),
...
CREATE POLICY "Usuarios podem ver proprio perfil" ON public.profiles
  FOR SELECT USING (auth.uid() = id);
DROP POLICY IF EXISTS "Usuarios podem inserir proprio perfil" ON public.profiles;
CREATE POLICY "Usuarios podem inserir proprio perfil" ON public.profiles
  FOR INSERT WITH CHECK (auth.uid() = id);
DROP POLICY IF EXISTS "Usuarios podem atualizar proprio perfil" ON public.profiles;
CREATE POLICY "Usuarios podem atualizar proprio perfil" ON public.profiles
  FOR UPDATE USING (auth.uid() = id) WITH CHECK (auth.uid() = id);
DROP POLICY IF EXISTS "Service role gerencia todos perfis" ON public.profiles;
CREATE POLICY "Service role gerencia todos perfis" ON public.profiles
  FOR ALL USING (auth.role() = 'service_role');

GRANT SELECT ON TABLE public.setores TO authenticated;
GRANT SELECT, INSERT, DELETE ON TABLE public.lixeira_acordos TO authenticated;
GRANT SELECT, INSERT, UPDATE ON TABLE public.profiles TO authenticated;
GRANT ALL ON TABLE public.setores, public.lixeira_acordos, public.profiles TO service_role;
