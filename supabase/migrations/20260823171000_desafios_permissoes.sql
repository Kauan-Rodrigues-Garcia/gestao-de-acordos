-- ============================================================================
-- 20260823171000_desafios_permissoes.sql
--
-- As duas chaves do modulo Desafios.
--
-- ## `analitico_sub_desafios`
--
-- A aba interna, na mesma familia de `analitico_sub_analitico`,
-- `analitico_sub_recebimento_diario` e `analitico_sub_colchao`. Nasce ligada
-- para todo mundo: uma gincana existe para ser vista por quem disputa, e um
-- placar que so a lideranca enxerga nao motiva ninguem.
--
-- Desligar esta chave para um cargo tira a aba dele — e e por ela que uma
-- operacao que nao queira o placar publico encerra a exposicao, sem deploy e
-- sem mexer em nenhuma regra de escopo do Analitico.
--
-- ## `desafios_configurar`
--
-- Criar, editar, ativar e encerrar campanha. Nasce em `ninguem`, o que NAO e
-- esquecimento: `administrador` e `super_admin` recebem `true` por regra do
-- resolvedor, entao a administracao ja funciona, e qualquer outra pessoa
-- precisa ser habilitada nominalmente. Quem configura decide meta e premio de
-- uma disputa com dinheiro no meio — e a decisao certa para essa chave.
--
-- E ela, e nao uma lista de cargo, que a RLS de `public.desafios` consulta
-- (migration 20260823170000).
--
...
AS $function$
  ...
  SELECT t.* FROM atalhos, LATERAL (VALUES
    ...
    -- Desafios (gincanas internas) — a aba vive dentro do Analitico
...
BEGIN
  FOR v_chave IN SELECT chave, padrao, explicita FROM public.fn_permissoes_catalogo()
                  WHERE chave IN ('analitico_sub_desafios',
                                  'desafios_configurar')
  LOOP
